<?php

namespace App\Http\Controllers\Petugas;

use App\Http\Controllers\Controller;
use App\Models\PengaduanModel;
use App\Models\MasyarakatModel;
use Illuminate\Support\Facades\Hash;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;
use Exception;

class PtMasyarakatController extends Controller
{
    public function index()
    {
        try {
            $masyarakat = MasyarakatModel::all();
            $pengaduanTanggapan = PengaduanModel::whereIn('status', ['diverifikasi', 'ditolak'])
                                        ->orderBy('updated_at', 'desc') 
                                        ->get();

            return view('petugas.masyarakat.masyarakat', compact('masyarakat', 'pengaduanTanggapan'));
        } catch (Exception $e) {
            return redirect()->back()->with('error', 'Terjadi kesalahan saat memuat data: ' . $e->getMessage());
        }
    }

    public function store(Request $request)
    {
        $request->validate([
            'nama_masyarakat' => 'required|string|max:255',
            'nik' => 'required|string|max:16',
            'email' => 'required|email',
            'telp' => 'required|string|max:15',
            'password' => 'required|min:8',
        ]);

        MasyarakatModel::create([
            'nama_masyarakat' => $request->nama_masyarakat,
            'nik' => $request->nik,
            'email' => $request->email,
            'telp' => $request->telp,
            'password' => bcrypt($request->password),
        ]);

        return redirect()->back()->with('success', 'Masyarakat berhasil ditambahkan.');
    }

    public function destroy($id)
    {
        try {
            $masyarakat = MasyarakatModel::findOrFail($id);
            $masyarakat->delete();

            return redirect()->back()->with('success', 'Masyarakat berhasil dihapus.');
        } catch (Exception $e) {
            return redirect()->back()->with('error', 'Gagal menghapus masyarakat: ' . $e->getMessage());
        }
    }

    public function update(Request $request, $id)
    {
        try {
            $request->validate([
                'nik' => 'required|string|max:16',
                'nama_masyarakat' => 'required|string|max:255',
                'email' => 'required|email',
                'telp' => 'nullable|string|max:15',
                'foto_profil' => 'nullable|image|mimes:jpeg,png,jpg|max:2048',
                'password' => 'nullable|string|min:8|confirmed',
            ]);

            $masyarakat = MasyarakatModel::findOrFail($id);
            $masyarakat->nik = $request->nik;
            $masyarakat->nama_masyarakat = $request->nama_masyarakat;
            $masyarakat->email = $request->email;
            $masyarakat->telp = $request->telp;

            if ($request->hasFile('foto_profil')) {
                if ($masyarakat->foto_profil && Storage::exists('public/foto_masyarakat/' . $masyarakat->foto_profil)) {
                    Storage::delete('public/foto_masyarakat/' . $masyarakat->foto_profil);
                }

                $fileName = time() . '_' . $request->file('foto_profil')->getClientOriginalName();
                $request->file('foto_profil')->storeAs('public/foto_masyarakat', $fileName);
                $masyarakat->foto_profil = $fileName;
            }

            if ($request->filled('password')) {
                $masyarakat->password = Hash::make($request->password);
            }

            $masyarakat->save();

            return redirect()->back()->with('success', 'Profil berhasil diperbarui.');
        } catch (Exception $e) {
            return redirect()->back()->with('error', 'Gagal memperbarui profil: ' . $e->getMessage());
        }
    }
}
